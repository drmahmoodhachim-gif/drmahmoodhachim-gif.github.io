...
# FIX 1: Use matrix() for noise to guarantee correct dimensions,
#         and wrap pmax() result back into matrix() to preserve dimensions
noise        <- matrix(sample(-3:3, n_genes * n_samples, replace = TRUE),
                       nrow = n_genes, ncol = n_samples)
count_matrix <- matrix(pmax(1, count_matrix + noise),
                       nrow = n_genes, ncol = n_samples)
...
# FIX 2: Calculate significance before subsetting so sig inherits it cleanly
res_df$significant <- ifelse(
  !is.na(res_df$padj) & res_df$padj < 0.05 & abs(res_df$log2FoldChange) > 1,
  "Yes", "No"
)

sig <- subset(res_df, significant == "Yes")
...
# --- Heatmap ---
# FIX 3: Use varianceStabilizingTransformation() instead of vst() —
#         vst() requires 1000+ genes and will error on small datasets
vsd <- varianceStabilizingTransformation(dds, blind = FALSE)
mat <- assay(vsd)

if (nrow(sig) > 0) {
  # FIX 4: Use sig$gene (character column) instead of rownames(sig)
  #         to reliably index into the VST matrix
  top_genes  <- sig$gene[1:min(20, nrow(sig))]
  mat_heat   <- mat[top_genes, , drop = FALSE]
  mat_scaled <- t(scale(t(mat_heat)))

  # FIX 5: Reverse RdBu so blue = low, red = high (standard heatmap convention)
  heatmap_colors <- rev(colorRampPalette(brewer.pal(9, "RdBu"))(100))
...
# FIX 6: Also save MA plot as PNG for easy viewing (consistent with other plots)
ggsave("05_deg_results/ma_plot.png", p_ma, width = 6, height = 5, dpi = 150)
...
if (nrow(sig) > 0) {
  ...
  p1 <- ggplot(pathway_enrichment,
               aes(x = reorder(pathway, deg_count), y = deg_count, fill = pathway)) +
  ...
  ggsave("06_systems_biology/regulation_summary.png", p2, width = 5, height = 4, dpi = 150)
...
cat("  - 05_deg_results/ma_plot.png\n")
...
cat("\nOpen the PNG files by double-clicking them.\n\n")
